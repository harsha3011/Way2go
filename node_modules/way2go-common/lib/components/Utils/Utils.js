...
Utils.normalizeLocale = function (locale) {
...
Utils.checkAbsoluteUrl = function (str) {
    var pat = /^https?:\/\//i;
    if (pat.test(str)) return true;else return false;
};

Utils.getWSConnectionStatus = function (ws) {
    var status = "";
...
Utils.setDefaultLocale = function () {
    ...
    if (localesSupportedArray.indexOf(language) == -1) {
        //set default locale if locale not supported
        storageObject.setValue(storageObject.options.LOCALE, constants.defaultLocale);
    }
    //end
};

Utils.getPageNameFromRelativePath = function (url) {
    //start - set default locale, if locale not supported
    var result;
    if (url != undefined) {
        try {
...
Utils.dataURItoBlob = function (dataURI, fileName) {
...
Utils.dataURLtoFile = function (dataurl, filename) {
...
Utils.convertDateToUTC = function (value) {
    return new Date(value.getTime() + value.getTimezoneOffset() * 60 * 1000);
};

Utils.convertDateFromUTC = function (value) {
    return new Date(value.getTime() - value.getTimezoneOffset() * 60 * 1000);
};
